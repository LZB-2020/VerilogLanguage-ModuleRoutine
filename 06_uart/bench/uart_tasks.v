...
task send_serial;
    ...
    begin
        ...
        if (parityenable == `PARITY_ON) begin
            ...
            // even parity
            if (paritytype == `PARITY_ODD) begin
...
task get_serial;
    ...
    begin
        ...
        if (serial_in != 0) begin
            ...
            if (parityenable == `PARITY_ON) begin
                if (paritytype == `PARITY_EVEN) begin
                    expected_parity = (nbits == `NBITS_7) ? (  ^get_serial_data[6:0] ) : (  ^get_serial_data[7:0] );
                end
                else begin
                    expected_parity = (nbits == `NBITS_7) ? (~(^get_serial_data[6:0])) : (~(^get_serial_data[7:0]));
                end
                if (expected_parity != serial_in) begin
